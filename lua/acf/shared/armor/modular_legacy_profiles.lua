...
local function CommonSpec(legacy, behavior)
	...
	if legacy.HEATMul then spec.heatDamageMultiplier = legacy.HEATMul end
	for key, value in pairs(behavior or {}) do spec[key] = value end
	return spec
end

local function CommonResolverConfig()
	return {
...
CHAConfig.breachUsesEffectiveness = false
...
-- These are internal failure-model parameters. The legacy coefficients above
-- remain unchanged: ceramic loses most of its strike-face benefit after damage,
-- while its backing continues to limit residual fragment energy.
Profile("Cer", "Ceramic", "Ceramic", "Ceramic is usually used as a material to ensure shells do not penetrate due to its high penetration resistance. Due to its frailty it is usually only used as a backing and is not meant to take the brunt of damage. Do not let it get penetrated or it will shatter.", 1955, Cer, { "brittle_strike_face", { id = "composite_backing", parameters = { residualDamageMultiplier = 0.7 } }, "spall_response", "failure_state" }, CommonSpec(Cer, { multiHitRetention = 0.2, degradation = 0.85 }), { impactHook = "ceramic_shatter", ductilityFactor = 1.25, ductilityBase = 4, ductilityScale = 1.5 })
local DUConfig = CommonResolverConfig(DU)
Profile("DU", "Depleted Uranium", "DU", "Heavy yet extremely effective armor. Though costly, a slab of this can stop just about anything. More effective at higher thicknesses.", 1970, DU, { "dense_metal", "homogeneous_metal", "spall_response" }, CommonSpec(DU), DUConfig)
...
-- ERA is an expendable tile: after its reactive activation it no longer
-- contributes ballistic resistance. Its legacy mass/effectiveness values stay
-- exactly as published to players.
Profile("ERA", "Explosive Reactive Armor", "ERA", "An explosive composite sandwiched between 2 plates. When penetrated the plate detonates damaging or even destroying the incoming shell degrading its performance. Explosive rounds can make short work of this material. This material is heavy compared to RHA and unlike other materials, will damage anything near the detonation.", 1955, ERA, { "reactive_tile", "shock_barrier", "spall_response", "failure_state" }, CommonSpec(ERA, { singleUse = true }), { impactHook = "era_detonation", ductilityFactor = 1.25, ductilityBase = 2, ductilityScale = 1.5 })
...
local RubConfig = CommonResolverConfig(Rub)
RubConfig.kineticBreachCaliberMultiplier = 10
RubConfig.stoppedResilience = Rub.Catchresiliance
RubConfig.threats = {
	AP = { breachCaliberMultiplier = 10, stoppedResilience = Rub.Catchresiliance },
	HE = { breachCaliberMultiplier = 10, stoppedResilience = Rub.HEresiliance },
	HESH = { breachCaliberMultiplier = 10, stoppedResilience = Rub.HEresiliance },
	Spall = { breachCaliberMultiplier = 1, effectiveness = Rub.spallresist, stoppedResilience = Rub.Catchresiliance },
	HEAT = { breachCaliberMultiplier = 1, stoppedResilience = Rub.HEATresiliance },
	THEAT = { breachCaliberMultiplier = 1, stoppedResilience = Rub.HEATresiliance },
	HEATFS = { breachCaliberMultiplier = 1, stoppedResilience = Rub.HEATresiliance },
	THEATFS = { breachCaliberMultiplier = 1, stoppedResilience = Rub.HEATresiliance }
}
Profile("Rub", "Rubber", "Rubber", "Another material that while useless against kinetic rounds, excels at stopping shaped charges and spall", 1955, Rub, { "elastomer_liner", "spall_response", "shock_barrier" }, CommonSpec(Rub, { spallCapture = 0.65, spallCaptureArealDensity = 18, spallCaptureVelocity = 1400, spallCaptureSpacing = 0.1 }), RubConfig)
-- Textolite is a rigid composite backing rather than a second strike face:
-- preserve residual-energy reduction while allowing cumulative damage.
Profile("Texto", "Textolite", "Textolite", "Fiberglass based material, this material provides decent protection agaisnt both chemical especially and kinetic rounds, while taking reduced damage from explosions.", 1955, Texto, { { id = "composite_backing", parameters = { residualDamageMultiplier = 0.7 } }, "spall_response", "failure_state" }, CommonSpec(Texto, { multiHitRetention = 0.55, degradation = 0.45, spallCapture = 0.35, spallCaptureArealDensity = 40, spallCaptureVelocity = 1800, spallCaptureSpacing = 0.05 }), { ductilityFactor = 1.25, ductilityBase = 2, ductilityScale = 1.5 })
