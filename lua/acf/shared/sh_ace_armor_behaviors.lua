...
ACE.ArmorSpecFields = {
	...
	heatDamageMultiplier = { label = "HEAT damage", unit = "×" },
...
function ACE.ArmorImpactPolicy.ConditionLoss(hitResult, penetration, losArmor, maxHealth)
	...
	local penetratingOutcome = hitResult.Outcome == "penetrated" or hitResult.Outcome == "breached"
	if ratio < threshold and not penetratingOutcome then return 0 end
	...
	local outcomeFactor = penetratingOutcome and 1 or severity * severity
...
local function ValidateResolverConfig(config)
	local allowed = {
		...
		kineticBreachCaliberMultiplier = true,
	...
	for _, key in ipairs({ "ductilityFactor", "ductilityBase", "ductilityScale", "breachCaliberMultiplier", "kineticBreachCaliberMultiplier", "stoppedResilience" }) do
...
function ACE.ValidateArmorSpec(spec)
	...
	local positive = {
		...
		"curve", "overmatchRatio", "kineticResilience", "chemicalResilience", "heResilience", "penetrationDamageMultiplier", "heatDamageMultiplier",
...
function ACE.ConfigureArmorMaterial(material, definition)
	...
	if definition.resolver == "modular" then
		...
		if SERVER then
			material.ArmorResolution = function(Entity, armor, losArmor, losArmorHealth, maxPenetration, FrArea, caliber, damageMult, Type, impactCondition)
				return ACE.ArmorResolvers.Modular(material, Entity, armor, losArmor, losArmorHealth, maxPenetration, FrArea, caliber, damageMult, Type, impactCondition)
			end
		end
	end
...
if SERVER then
	...
	function ACE.ArmorResolvers.Modular(material, Entity, armor, losArmor, losArmorHealth, maxPenetration, FrArea, caliber, damageMult, Type, impactCondition)
		...
		local threat, effectiveness, resilience = ThreatValues(material, Type)
		...
		if HEATTypes[Type] then damageMultiplier = damageMultiplier * (spec.heatDamageMultiplier or 1) end
		...
		if ductility == nil then
			ductility = (Entity and Entity.ACF and tonumber(Entity.ACF.Ductility)) or 0
			ductility = ductility * (resolverConfig.ductilityFactor or 1)
		end
		ductility = math.Clamp(ductility, -1, 1)
		...
		local breachDuctilityMultiplier = resolverConfig.breachDuctility == false and 1 or ductilityMultiplier
		local effectiveArmor = armor ^ curve
		local effectiveLosArmor = losArmor ^ curve
		local breachMultiplier = threatConfig.breachCaliberMultiplier or resolverConfig.breachCaliberMultiplier
		if threat == "kinetic" then breachMultiplier = threatConfig.breachCaliberMultiplier or resolverConfig.kineticBreachCaliberMultiplier or breachMultiplier end
		local breachCaliber = caliber * (breachMultiplier or 1)
		local breachProbability = math.Clamp((breachCaliber / effectiveArmor / effectiveness - 1.3) / (overmatchRatio - 1.3), 0, 1)
		local breachEffectiveness = resolverConfig.breachUsesEffectiveness == false and 1 or effectiveness
		local breachArmor = effectiveArmor * breachEffectiveness
		...
		if breachProbability > ImpactRandom() and maxPenetration > breachArmor then
			...
			return ImpactResult(
				FrArea * resilience * damageMult * breachDuctilityMultiplier * damageMultiplier,
		...
		local stoppedResilience = threatConfig.stoppedResilience or resolverConfig.stoppedResilience or resilience
		return ImpactResult(
			(penetration / losArmorHealth / effectiveness) * FrArea * stoppedResilience * damageMult * ductilityMultiplier * damageMultiplier,
			0,
			1,
			"stopped",
			maxPenetration
