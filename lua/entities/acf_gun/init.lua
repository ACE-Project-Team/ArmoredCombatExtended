...
do
	...
	function ACE_MakeGun(Owner, Pos, Angle, Id)
...
function ENT:Think()
	...
	if ACE.CurTime > self.NextLegalCheck then
		...
		-- check gun is legal
		self.Legal, self.LegalIssues = ACE.RequireLegal(self, self.Model, math.Round(self.Mass,2), self.ModelInertia, nil, true)
	...
	if self.CurrentRecoil > 0 then
		...
		ACE_KEShove(self, self:GetPos() , Dir , self.KERecoil * self.CurrentRecoil )
