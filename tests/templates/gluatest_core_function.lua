...
local function returns(name, callback, expected)
	return {
		name = name,
		func = function(State) expect(callback(State)).to.equal(expected) end
	}
end
...
local function greaterThan(name, callback, expected)
	return {
		name = name,
		func = function(State) expect(callback(State)).to.beGreaterThan(expected) end
	}
end
...
local function lessThan(name, callback, expected)
	return {
		name = name,
		func = function(State) expect(callback(State)).to.beLessThan(expected) end
	}
end
...
return {
	...
	afterEach = function(State)
		...
		if State.InvalidSubject then CleanupSmallTestFixture(State.InvalidSubject) end
	end,
	cases = {
		returns("returns the normal result", function(State) return ACE.Area.Function(State.Subject) end, ExpectedValue),
		greaterThan("returns a larger result for the stronger fixture", function(State) return ACE.Area.Function(State.StrongerSubject) end, ExpectedValue),
		lessThan("returns a smaller result for the weaker fixture", function(State) return ACE.Area.Function(State.WeakerSubject) end, ExpectedValue),
		changesWhen("responds when the input changes", function(State) return ACE.Area.Function(State.Subject) end, function(State)
			State.Subject.Input = ChangedInput
			return ACE.Area.Function(State.Subject)
		end),
		{
			name = "handles invalid input safely",
			func = function() expect(ACE.Area.Function(nil)).to.beFalse() end
		},
		{
