...
MIGRATED_EVIDENCE = {
    "ACE.PointFlush": ("partial", "native direct/scheduled point parity and teardown; dispatch timing sample only; dynamic mutation sensitivity open"),
    "ACE.PeriodicCleanup": ("partial", "native activation/disable/teardown; active-contraption scaling open"),
    "ACE.PermissionModeThink": ("passing", "native activation/teardown and approximately 10 ms cadence"),
    "ACE.SafezoneTransition": ("partial", "native activation, approximately 15.15 ms cadence, entry/exit delivery, and teardown passed; relative hook-order parity remains open"),
    "ACE.SafezoneVisualization": ("partial", "LuaJIT delayed-callback parity, fallback invalidation, reload, and teardown; native visualization packet ordering open"),
    "ACE.ContraptionLegalCheck": ("partial", "LuaJIT per-entity cooldown parity, validity guard, reload, and fallback teardown; native warning-scan parity open"),
    "ACE.Wind": ("partial", "native activation/disable/teardown; full reset broadcast/cadence parity open"),
    "ACE.RenderPropDamage": ("partial", "LuaJIT coalescing/cadence/reload/teardown; native packet parity open"),
    "ACE.WindSensorThink": ("partial", "LuaJIT cadence, fallback, removal, reload, and key-ownership coverage; native v34 two-sensor cadence, one additional fallback update each, resume, output-snapshot parity, isolation, and deferred teardown pass; connected Wire packet ordering and larger-scale load scaling open"),
    "ACE.ScalableResync": ("partial", "LuaJIT reverse-order live-table parity, one-per-tick pacing, replacement, disconnect cancellation, reload, and enable/disable fallback coverage; native v36 real-bot recipient/order/toggle/teardown gate passed with 14.971-15.379 ms scheduled spacing and 15.009-30.029 ms fallback spacing; direct net receiver and disconnect cancellation remain open"),
    "ACE.VHeatSourceThink": ("partial", "LuaJIT fixed-step heat/clamp, Wire/overlay ordering, two-entity independence, phase-preserving fallback, removal, and enabled reload coverage; native v46 first-step parity for the first scheduled source, real-entity cadence/removal, and clean scheduler disable/enable teardown path passed with 106.613 ms first spacing and 105.530-107.074 ms survivor spacing; post-enable recurrence, connected Wire packet, and larger-scale load scaling remain open"),
    "ACE.GForceMeterThink": ("partial", "LuaJIT fixed-step CalculateGForce/output/overlay ordering, independent entities, fallback phase, removal, and enabled reload coverage; native v51 real-entity stationary first-step parity at GForce=1, 59.486-61.438 ms survivor cadence, removal isolation, and clean disable/enable teardown passed; dynamic physics parity, connected Wire packet, and larger-scale load scaling remain open"),
    "ACE.EntityRemoval": ("partial", "LuaJIT one-shot heap/fallback parity for debris and flare lifetimes, replacement, cancellation, invalid-entity guard, and disable restoration pass; native v2 explicitly observed fallback timer versus heap path selection, disable/re-enable lifecycle, deferred cancellation record/node removal, and post-cleanup quiescence; see docs/ace_scheduler_debris_native_v2.json; larger-scale timer/dispatch cost remains open"),
    "ACE.GunAutoSound": ("partial", "LuaJIT heap/fallback parity for independent delayed gun sounds, argument preservation, invalid-gun guard, invalidation-before-delivery, reload, and disable restoration pass; native sound delivery and larger-scale dispatch cost remain open"),
    "ACE.AmmoCookoffFlash": ("partial", "LuaJIT heap/fallback parity for independent delayed flashes, dynamic position capture, invalid-ammo guard, invalidation-before-reload cleanup, and disable restoration pass; native effect delivery and larger-scale dispatch cost remain open"),
    "ACE.DamageDetonationEffect": ("partial", "LuaJIT heap/fallback parity for independent delayed detonation effects, argument preservation, reload, and disable restoration pass; native effect delivery and larger-scale dispatch cost remain open"),
    "ACE.FlareThink": ("partial", "LuaJIT fixed-step signature parity, native fallback phase, underwater stop/frozen radar signature, disable, unregister, and enabled-reload coverage pass; native v2 real-entity cadence, disable-to-zero, re-enable fallback, and removal quiescence pass (docs/ace_scheduler_flare_native_v2.json); native underwater behavior and larger-scale cadence cost remain open"),
    "ACE.SonarTravelSound": ("partial", "LuaJIT behavioral parity plus native v1 real-entity adapter evidence for heap/timer selection, disable/re-enable fallback, due-time delivery, invalid-base cleanup, and post-cleanup quiescence; audible delivery and larger-scale dispatch cost remain open; see docs/ace_scheduler_sonar_native_v1.json"),
    "ACE.SonarPingExpiry": ("partial", "LuaJIT and native v6 heap/fallback parity for stale-cache expiry, per-contraption/ping coalescing, invalid-base rejection including same-frame deletion, replacement, due cleanup, and teardown pass; native scale v1 drained 1,000/5,000/10,000 records in 3.485/12.874/25.142 ms dispatch time; whole-server lag attribution remains open; see docs/ace_scheduler_sonar_ping_native_scale_v1.json"),
}
